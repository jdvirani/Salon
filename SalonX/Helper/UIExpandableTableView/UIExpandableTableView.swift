...
class UIExpandableTableView : UITableView, HeaderViewDelegate {
    ...
    func headerViewOpen(_ section: Int) {
        ...
        let numberOfRows:Int = (self.dataSource?.tableView(self, numberOfRowsInSection: section))!
        ...
        if indexesPathToInsert.count > 0 {
            self.beginUpdates()
            self.insertRows(at: indexesPathToInsert, with: .fade)
            self.endUpdates()
        }
    }
    ...
    func headerViewClose(_ section: Int) {
        ...
        if indexesPathToDelete.count > 0 {
            self.beginUpdates()
            self.deleteRows(at: indexesPathToDelete, with: .fade)
            self.endUpdates()
        }
    }
}
